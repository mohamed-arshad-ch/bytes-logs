...
import type { Product } from "@/lib/db-service"
import {
  getClientsForUser,
  getProductsForUser,
...
export default function TransactionForm({ mode, initialData }: TransactionFormProps) {
  ...
  const handleProductSearch = async (id: string, query: string) => {
    ...
    if (query.length > 1) {
      ...
      try {
        // Fetch products from database based on search query
        const response = await searchProductsForUser(query)
        if (response.success) {
  ...
  const handleSubmit = async (action: "save") => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        response = await updateTransaction(formData.transactionId, transactionData)
      }

      if (response.success) {
        toast({
          title: "Success",
          description: response.message || `Transaction ${mode === "create" ? "created" : "updated"} successfully`,
        })
        router.push("/admin/transactions")
      } else {
        toast({
          title: "Error",
          description: response.error || `Failed to ${mode} transaction`,
          variant: "destructive",
        })
      }
    } catch (error) {
