...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-gradient-to-b from-[#3A86FF]/10 to-transparent rounded-bl-full" />

      {/* Abstract Admin Pattern */}
      <div className="absolute bottom-10 left-10 opacity-5">
        <div className="grid grid-cols-3 gap-4">
          {[...Array(9)].map((_, i) => (
            <div key={i} className="w-16 h-16 border border-[#3A86FF] rounded-md" />
          ))}
        </div>
      </div>

      {/* Login Card */}
      <div className="w-full max-w-md bg-white rounded-lg shadow-[0_4px_15px_rgba(0,0,0,0.08)] p-8 relative z-10">
        {/* Logo */}
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-lg font-semibold mt-2 font-poppins">Admin Portal</h2>
          <p className="text-[#6B7280] text-sm mt-1 font-poppins">Sign in to manage your business</p>
        </div>
