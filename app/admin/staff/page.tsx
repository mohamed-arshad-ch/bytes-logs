...
export default function AdminStaff() {
  ...
  const fetchStaff = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Fetch total paid amount for each staff member
        const staffWithTotalPaid = await Promise.all(
          formattedStaff.map(async (staffMember) => {
            try {
              const totalPaidResponse = await getStaffTotalPaidAmount(staffMember.id)
              return {
                ...staffMember,
                total_paid: totalPaidResponse.success ? totalPaidResponse.data : 0,
              }
            } catch (error) {
              console.error(`Error fetching total paid for staff ${staffMember.id}:`, error)
              return {
                ...staffMember,
                total_paid: 0,
              }
        ...
        setStaff(staffWithTotalPaid)
      } else {
        setToast({ message: response.error || "Failed to fetch staff", type: "error" })
      }
    } catch (error) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await searchStaffMembers(searchQuery)
      if (response.success) {
        // Ensure payment_rate is a number for all staff members
        const formattedStaff = response.data.map((staff) => ({
          ...staff,
        ...
        setToast({ message: response.error || "Failed to search staff", type: "error" })
      }
    } catch (error) {
  ...
  const handleFilter = async () => {
    try {
      const response = await filterStaffMembers(roleFilter, statusFilter)
      if (response.success) {
        // Ensure payment_rate is a number for all staff members
        const formattedStaff = response.data.map((staff) => ({
          ...staff,
        ...
        setToast({ message: response.error || "Failed to filter staff", type: "error" })
      }
    } catch (error) {
  ...
  const handleViewDetails = async (id: number) => {
    ...
    try {
      ...
      if (response.success) {
        // Ensure payment_rate is a number
        const formattedStaff = {
          ...response.data,
          payment_rate: Number(response.data.payment_rate),
        }

        // Get total paid amount
        const totalPaidResponse = await getStaffTotalPaidAmount(id)
        if (totalPaidResponse.success) {
          formattedStaff.total_paid = totalPaidResponse.data
        }
  ...
  const handleDeleteStaff = async () => {
    ...
    try {
      const response = await deleteStaffMember(staffToDelete)
      if (response.success) {
  ...
  const handleActivatePortalAccess = async (staff: Staff) => {
    ...
    try {
      const result = await createStaffPortalAccess(staff.id)
      if (result.success) {
